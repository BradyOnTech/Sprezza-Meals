...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(sql`DROP TABLE IF EXISTS "reviews_images" CASCADE;`)

  // Remove reviews relationship from meals_rels if it exists
  ...
  await db.execute(sql`
    ALTER TABLE "meals_rels" ADD COLUMN "reviews_id" INTEGER REFERENCES "reviews"("id") ON DELETE CASCADE;
  `)

  // Create indexes
  await db.execute(sql`CREATE INDEX "reviews_meal_idx" ON "reviews"("meal");`)
  await db.execute(sql`CREATE INDEX "reviews_user_idx" ON "reviews"("user_id");`)
  await db.execute(
